...
describe.each(MODE_REGISTRY.map((m) => [m.id, m.label]))('%s (%s)', (id) => {
  ...
  it('handles extreme dimensions', () => {
    const wide = generateArt(buildModeSettings({ ...SHARED, width: 3000, height: 100 }, modeId, 'dark'))
    const tall = generateArt(buildModeSettings({ ...SHARED, width: 100, height: 3000 }, modeId, 'dark'))
  ...
  it('handles minimum dimensions', () => {
    const tiny = generateArt(buildModeSettings({ ...SHARED, width: 64, height: 64 }, modeId, 'dark'))
