...
export const voronoiEdgesDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return { ...settings, cellCount: clamp(Math.round(settings.cellCount), 8, 60), jitter: clamp(Math.round(settings.jitter), 0, 100), thickness: clamp(Math.round(settings.thickness), 1, 4) }
  },
}
