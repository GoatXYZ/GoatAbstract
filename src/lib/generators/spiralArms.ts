...
export const spiralArmsDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return { ...settings, armCount: clamp(Math.round(settings.armCount), 1, 8), turns: clamp(Math.round(settings.turns), 1, 6), spread: clamp(Math.round(settings.spread), 0, 100) }
  },
}
