...
export const lightningBoltsDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return {
      ...settings,
      boltCount: clamp(Math.round(settings.boltCount), 1, 8),
      branches: clamp(Math.round(settings.branches), 0, 4),
      jaggedness: clamp(Math.round(settings.jaggedness), 10, 100),
    };
  },
};
