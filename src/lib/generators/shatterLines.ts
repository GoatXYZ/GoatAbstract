...
export const shatterLinesDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return { ...settings, impactCount: clamp(Math.round(settings.impactCount), 1, 5), rayCount: clamp(Math.round(settings.rayCount), 6, 24), ringCount: clamp(Math.round(settings.ringCount), 0, 6) }
  },
}
