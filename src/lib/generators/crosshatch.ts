...
const generate = (settings: ArtSettings): WavePath[] => {
  ...
  for (let layer = 0; layer < layerCount; layer++) {
    const layerAngle =
      (angle * Math.PI) / 180 + (layer * Math.PI) / layerCount + (random() - 0.5) * 0.15;
...
export const crosshatchDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return {
      ...settings,
      layerCount: clamp(Math.round(settings.layerCount), 2, 6),
      density: clamp(Math.round(settings.density), 8, 40),
      angle: clamp(Math.round(settings.angle), 0, 90),
    };
  },
};
