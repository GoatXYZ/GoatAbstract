...
export const dotMatrixDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return { ...settings, columns: clamp(Math.round(settings.columns), 5, 40), sizeRange: clamp(Math.round(settings.sizeRange), 1, 10), chaos: clamp(Math.round(settings.chaos), 0, 100) }
  },
}
