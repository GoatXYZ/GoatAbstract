...
export const smokePlumesDefinition: ModeDefinition = {
  ...
  normalize(settings: ArtSettings): ArtSettings {
    ...
    return {
      ...settings,
      plumeCount: clamp(Math.round(settings.plumeCount), 1, 6),
      turbulence: clamp(Math.round(settings.turbulence), 10, 100),
      segments: clamp(Math.round(settings.segments), 6, 20),
    };
  },
};
