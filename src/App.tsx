...
const parseUrlSettings = (): ArtSettings | null => {
  try {
    ...
    const base = buildModeSettings({ width, height, seed }, raw.mode as ModeId, theme);
...
function RenderMode({ settings }: { settings: ArtSettings }) {
  ...
  return <div className="render-mode" dangerouslySetInnerHTML={{ __html: art.svg }} />;
}
...
function Editor() {
  ...
  const [settings, setSettings] = useState<ArtSettings>(() => {
    ...
    return {
      ...buildModeSettings(
        { width: 1600, height: 900, seed: createRandomSeed() },
        randomMode,
        getInitialTheme(),
      ),
      ...palette,
    ...
    setSettings((current) => {
      if (aspectLocked && key === "width" && typeof value === "number") {
        return {
          ...current,
          width: value,
          height: Math.max(64, Math.round(value / aspectRatioRef.current)),
        };
      }
      if (aspectLocked && key === "height" && typeof value === "number") {
        return {
          ...current,
          width: Math.max(64, Math.round(value * aspectRatioRef.current)),
          height: value,
        };
      }
  ...
  useEffect(() => {
    keyHandlerRef.current = (e: KeyboardEvent) => {
      ...
      if (mod && e.key === "z" && !e.shiftKey) {
        ...
        if (idx < MODE_REGISTRY.length) {
          e.preventDefault();
          handleModeChange(MODE_REGISTRY[idx].id);
        }
      } else if (!mod && !e.shiftKey && e.key === "0" && MODE_REGISTRY.length >= 10) {
        e.preventDefault();
        handleModeChange(MODE_REGISTRY[9].id);
      }
    };
  ...
  return (
    <div className="app-shell">
      ...
      {fullscreen && <FullscreenOverlay svgMarkup={art.svg} onClose={() => setFullscreen(false)} />}
      {exploring && (
        <SeedExplorer
          settings={settings}
